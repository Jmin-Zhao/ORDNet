...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        print(val_conmat)
        with open(os.path.join(save_path, "demo.txt"), 'a') as f:
            f.writelines(
                '[epoch %d] train_loss: %.3f  train_acc: %.3f  val_acc: %.3f  p1: %.3f p2: %.3f  r1: %.3f r2: %.3f  f11: %.3f f12: %.3f' % (
                epoch + 1, mean_loss, train_acc, val_acc, val_p[0], val_p[1], val_r[0], val_r[1], val_f1[0], val_f1[1]) + '\n')
        with open(os.path.join(save_path, "demo.txt"), 'a') as f:
        ...
        # # print(val_conmat)
        # with open(os.path.join(save_path, "sum_sae_log.txt"), 'a') as f:
        #     f.writelines(
        #         '[epoch %d] train_loss: %.3f  train_acc: %.3f ' % (
        #             epoch + 1, mean_loss, train_acc) + '\n')
        ...
        if val_acc > best_acc:
            ...
            best_epoch = epoch
        # if epoch % 10 == 0:
            torch.save(model.state_dict(), os.path.join(save_path, "demo-" + str((epoch//10 + 1)*10) + ".pth"))
        if epoch % 10==0:
            best_acc = 0
            best_epoch = 0
        best_model_weights = copy.deepcopy(model.state_dict())
