...
# pytorch封装卷积层
class RHD(nn.Module):
    def __init__(self, expand_planes: int):
        super(HDC, self).__init__()

        self.conv1 = nn.Conv2d(in_channels=expand_planes, out_channels=expand_planes, kernel_size=3, stride=1, padding=1, dilation=1)
        self.bn1 = nn.BatchNorm2d(expand_planes)
        self.relu1 = nn.ReLU(inplace=True)

        self.conv2 = nn.Conv2d(in_channels=expand_planes, out_channels=expand_planes, kernel_size=3, stride=1, padding=2, dilation=2)
        self.bn2 =nn.BatchNorm2d(expand_planes)
        self.relu2 = nn.ReLU(inplace=True)

        self.conv3 = nn.Conv2d(in_channels=expand_planes, out_channels=expand_planes, kernel_size=3, stride=1, padding=3, dilation=3)
        self.bn3 =nn.BatchNorm2d(expand_planes)
        self.relu3 = nn.ReLU(inplace=True)

        self.conv4 = nn.Conv2d(in_channels=expand_planes, out_channels=expand_planes, kernel_size=3, stride=1, padding=1, dilation=1)
        self.bn4 =nn.BatchNorm2d(expand_planes)
        self.relu4 = nn.ReLU(inplace=True)

        self.conv5 =nn.Conv2d(in_channels=expand_planes, out_channels=expand_planes, kernel_size=3, stride=1, padding=2, dilation=2)
        self.bn5 =nn.BatchNorm2d(expand_planes)
        self.relu5 = nn.ReLU(inplace=True)

        self.conv6 =nn.Conv2d(in_channels=expand_planes, out_channels=expand_planes, kernel_size=3, stride=1, padding=3, dilation=3)
        self.bn6 =nn.BatchNorm2d(expand_planes)
...
class MobileNetV3(nn.Module):
    ...
    def __init__(self, inverted_residual_setting: List[InvertedResidualConfig], last_channel: int,  num_classes: int = 1000,
        ...
        layers.append(RHD(lastconv_output_c))
        self.features = nn.Sequential(*layers)

        # 增加HDC
        # self.hdc = HDC(lastconv_output_c)
...
"""
def mobilenet_v3_rhd123(num_classes: int = 1000, reduced_tail: bool = False):
